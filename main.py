...
num_points = 10000  # you can chage this and make sure my program works for lower number of particles
...
# Ploting
plt.figure()
plt.title('Contour plot for $P_{nn}=1$')
plt.contourf(grid)
plt.figure()
plt.title('Scatter plot for $P_{nn}=1$')
plt.scatter(yy, xx)
plt.show()
t1 = time()
...
grid_clear()
R_max = 20
# Creating the simulation for P_n= 0.3
simulate(num_points, 0.3)
# Save cluster 2
cluster2 = []
for i in Cluster:
    cluster2.append(i)
for i in Cluster:
    ...
    yy.append(i.y)
plt.figure()
plt.title('Contour plot for $P_{nn}=0.3$')
plt.contourf(grid)
plt.figure()
plt.title('Scatter plot for $P_{nn}=0.3$')
plt.scatter(yy, xx)
plt.show()
t2 = time()
